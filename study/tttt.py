...
def filter_contours(contours, min_area, max_area, min_ratio, max_ratio,
                    min_bbox_area=0, max_bbox_area=999999999):
    """
    輪郭を面積・縦横比・矩形面積でフィルタリング
    ...
    Args:
        ...
        min_ratio     : 縦横比の最小値
        max_ratio     : 縦横比の最大値
        min_bbox_area : 最小矩形面積（幅×高さ px²）
        max_bbox_area : 最大矩形面積（幅×高さ px²）
...
def detect(img_gray, method='otsu', invert=False, blur_size=3,
    ...
    contours_valid, info = filter_contours(
        contours, min_area, max_area, min_ratio, max_ratio,
...
def print_stats(info, img_shape):
    ...
    bbox_areas = [i['bbox_area'] for i in info]
    ratios     = [i['ratio']     for i in info]
    print(f"  輪郭面積  最小={min(areas):.0f}  最大={max(areas):.0f}  平均={np.mean(areas):.0f}")
    print(f"  矩形面積  最小={min(bbox_areas):.0f}  最大={max(bbox_areas):.0f}  平均={np.mean(bbox_areas):.0f}")
...
def interactive_mode(img_bgr):
    ...
    cv2.createTrackbar('最小輪郭面積',  win, 100,  5000,  lambda x: None)
    cv2.createTrackbar('最大輪郭面積',  win, 5000, 50000, lambda x: None)
    cv2.createTrackbar('最小矩形面積',  win, 0,    5000,  lambda x: None)
    cv2.createTrackbar('最大矩形面積',  win, 9999, 99999, lambda x: None)
    ...
    while True:
        ...
        min_area      = cv2.getTrackbarPos('最小輪郭面積',  win)
        max_area      = cv2.getTrackbarPos('最大輪郭面積',  win)
        min_bbox_area = cv2.getTrackbarPos('最小矩形面積',  win)
        max_bbox_area = cv2.getTrackbarPos('最大矩形面積',  win)
        ...
        contours, info, binary, _ = detect(
            ...
            min_bbox_area=min_bbox_area, max_bbox_area=max(1, max_bbox_area)
        )
...
def main():
    ...
    parser.add_argument('--min-area',      type=int,   default=100,
                        help='最小輪郭面積フィルタ（px² default:100）')
    parser.add_argument('--max-area',      type=int,   default=50000,
                        help='最大輪郭面積フィルタ（px² default:50000）')
    parser.add_argument('--min-bbox-area', type=int,   default=0,
                        help='最小矩形面積フィルタ 幅×高さ（px² default:0=無効）')
    parser.add_argument('--max-bbox-area', type=int,   default=999999999,
                        help='最大矩形面積フィルタ 幅×高さ（px² default=無効）')
